// Code Systems
Alias:   $SCT = http://snomed.info/sct
Alias:  $LOINC = http://loinc.org
Alias:   $AIClinStatus = http://terminology.hl7.org/CodeSystem/allergyintolerance-clinical
Alias:   $AIVerStatus = http://terminology.hl7.org/CodeSystem/allergyintolerance-verification
Alias:   $IdType = http://terminology.hl7.org/CodeSystem/v2-0203
Alias:   $ObsCat = http://terminology.hl7.org/CodeSystem/observation-category
Alias:  $CondClinStatus = http://terminology.hl7.org/CodeSystem/condition-clinical
Alias:  $CondVerStatus = http://terminology.hl7.org/CodeSystem/condition-ver-status
Alias:  $CondCat = http://terminology.hl7.org/CodeSystem/condition-category
Alias:  $AICriticality = http://hl7.org/fhir/allergy-intolerance-criticality
Alias:  $ObsInterpretation = http://terminology.hl7.org/CodeSystem/v3-ObservationInterpretation
Alias: 	$ListCode = http://hl7.org/fhir/ValueSet/list-example-codes